...
import {
  ...
  getPurchaseOrderStatusIcon,
  getPurchaseOrderStatusLabel,
} from '@/lib/purchaseOrderStatusDisplay';
import { PurchaseOrderStatus, type PurchaseOrderStatusOption } from '@/types/api/purchaseOrders';

interface PurchaseOrderLifecycleStepperProps {
  currentStatus: number;
  statuses: PurchaseOrderStatusOption[];
  isLoading?: boolean;
}

export function PurchaseOrderLifecycleStepper({
  currentStatus,
  statuses,
  isLoading = false,
}: PurchaseOrderLifecycleStepperProps) {
  ...
  return (
    <Card className="border shadow-xl shadow-slate-200/50 bg-card overflow-hidden rounded-3xl">
      <CardHeader className="bg-slate-50/80 border-b py-4 flex flex-row items-center justify-between gap-3">
        ...
        {isCancelled && (
          <Badge className={cn('font-bold', getPurchaseOrderStatusClassName(PurchaseOrderStatus.Cancelled))}>
            {getPurchaseOrderStatusLabel(PurchaseOrderStatus.Cancelled)}
          </Badge>
        )}
      </CardHeader>
      <CardContent className="p-0">
        ...
        >
          {steps.map((step) => {
            ...
            const Icon = getPurchaseOrderStatusIcon(step.value);
              ...
              >
                ...
                <div className="flex flex-col items-center text-center">
                  <span className={cn('text-[10px] font-black uppercase tracking-widest leading-none mb-1', colorClass)}>
                    {getPurchaseOrderStatusLabel(step.value)}
                  </span>
