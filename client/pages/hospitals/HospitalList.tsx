...
import { useGetHospitals, useDeleteHospital, hospitalKeys } from '@/hooks/useHospitals';
...
export default function HospitalList() {
  ...
  const { data: hospitalsResponse, isPending: isLoadingHospitals, error: hospitalsError } = useGetHospitals({
    pageNumber,
    pageSize,
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 p-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-red-700">{hospitalsError.userMessage}</p>
              <Button
                size="sm"
                variant="outline"
                onClick={() => queryClient.refetchQueries({ queryKey: hospitalKeys.lists() })}
                className="mt-2"
              >
                Try Again
              </Button>
      ...
      {!isLoadingHospitals && hospitals.length === 0 && !hospitalsError && (
        <div className="rounded-lg border border-dashed p-8 text-center">
          ...
          <p className="text-muted-foreground">
            {searchTerm ? 'Try adjusting your search criteria' : 'Add your first hospital customer to get started'}
          </p>
        </div>
      )}
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Showing {(pageNumber - 1) * pageSize + 1} to {Math.min(pageNumber * pageSize, totalCount)} of {totalCount} hospitals
            </div>
            <div className="flex gap-2">
              <Button
                variant="outline"
