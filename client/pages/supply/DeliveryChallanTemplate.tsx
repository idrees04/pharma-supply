      ...
      >
        ...
        <div className="mb-8 grid grid-cols-2 gap-8">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Deliver to</p>
            <p className="mt-2 text-base font-bold text-slate-900">
              {challan.hospitalName ?? '—'}
            </p>
            {challan.hospitalRegistrationNumber ? (
              <p className="mt-1 text-xs text-slate-600">
                Hospital No: <span className="font-mono">{challan.hospitalRegistrationNumber}</span>
              </p>
            ) : null}
            {challan.hospitalAddress ? (
              <p className="mt-1 text-xs text-slate-600">{challan.hospitalAddress}</p>
            ) : null}
            {challan.shippingAddress ? (
              <p className="mt-2 text-xs text-slate-600">
                Ship to: {challan.shippingAddress}
              </p>
            ) : null}
            {challan.hospitalPhone ? (
              <p className="mt-1 text-xs text-slate-600">Tel: {challan.hospitalPhone}</p>
            ) : null}
          </div>
          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <p className="font-bold text-slate-900">
                {challan.supplyOrderNumber ?? `SO #${challan.supplyOrderId}`}
              </p>
            </div>
            <div>
        ...
        <div className="mb-6 rounded-lg border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
          ...
          <p className="mt-1 whitespace-pre-wrap">{challan.notes?.trim() ? challan.notes : '—'}</p>
        </div>
        ...
        <p className="mt-6 text-center text-[10px] text-slate-400">
          Document for dispatch verification only (no pricing shown).
        </p>
      </div>
    );
  },
